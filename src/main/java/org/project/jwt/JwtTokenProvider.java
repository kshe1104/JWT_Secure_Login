...
import org.project.securelogin.repository.JwtTokenRedisRepository;
...
public class JwtTokenProvider {
    ...
    //JWT 토큰 생성
    private String generateToken(Authentication authentication, long accessTokenValiditySeconds) {
    ...
    //JWT 토큰의 유효성 검사
    // 유효한 토큰일 경우 true, 그렇지 않으면 false 리턴함
...
// JWT 토큰에서 사용자의 이름을 추출
    public String getEmail(String token){
        return Jwts.parser().setSigningKey(secretKey).parseClaimsJws(token).getBody().getSubject();
    }

    //HttpServletRequest에서 Authorization 헤더에서 JWT 토큰을 추출함
